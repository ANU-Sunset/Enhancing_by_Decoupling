...
def save_loss_plot(results, path='./', num_epochs=10, start_epoch=0, vae=True):
    ...
    tl_line1 = ax.plot(results['train_rec_loss'], label='training AE_testing_experiment loss', color='red')
    vl_line1 = ax.plot(results['val_rec_loss'], label='validation AE_testing_experiment loss', color='red', linestyle='dashed')
...
def save_checkpoint(model, optimizer, epoch, prefix="", path='./'):  # TODO
    model_out_path = path + "/" + prefix + "model.pth"
    state = {"epoch": epoch, "model": deepcopy(model.state_dict()), "optimizer": deepcopy(optimizer.state_dict())}
...
def train_vae(dataset='cifar10', path='./', z_dim=128, batch_size=32, num_workers=4, lr=1e-3,
              start_epoch=0,
              num_epochs=10, num_row=8,
              seed=-1, device=torch.device("cpu"), test_iter=1000, gamma=1, vae=True):
    if dataset == 'mnist':
        image_size = 28
        channels = [64, 128]
        ch = 1
        ...
        val_loader = DataLoader(MNIST('./data', train=False, transform=transforms.ToTensor()), batch_size=batch_size,
                                shuffle=True, **kwargs)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for x, _ in train_loader:
            ...
            # forward
            mu, logvar, z, y = model(x)
            ## loss
            # AE_testing_experiment loss
            # recon_loss_type="mse"
            n, c, h, w = x.shape
            loss_rec = 0.5 * calc_reconstruction_loss(x, y) / gamma + h * w * np.log(
        ...
        # save AE_testing_experiment images
        if cur_iter % test_iter == 0:
            _, _, _, rec_det = model(x)
            if dataset == 'celeb256':
        ...
        if min_val_loss > (ep_val_loss_rec + ep_val_loss_kl):
            prefix = dataset + "_zdim_" + str(z_dim) + "_VAE_gamma_" + str(gamma) + "_"
            save_checkpoint(model, optimizer, epoch, prefix, path=path)
...
if __name__ == '__main__':
    ...
    try:
        # MNIST
        # train_vae(dataset='mnist', z_dim=32, batch_size=128, num_workers=4, lr=1e-3,
        #                     start_epoch=0, num_epochs=10, device=device,
        #                     num_row=4, test_iter=1, gamma=0.5)
        ...
        ## VAE
        # train_vae(dataset='cifar10', z_dim=128, batch_size=32, num_workers=4, lr=1e-3,
        #                     start_epoch=0, num_epochs=101, device=device,
        #                     num_row=4, test_iter=10, gamma=0.01)

        # train_vae(dataset='cifar10', z_dim=128, batch_size=32, num_workers=4, lr=1e-3,
        #                     start_epoch=0, num_epochs=101, device=device,
        #                     num_row=4, test_iter=10, gamma=0.1)

        # train_vae(dataset='cifar10', z_dim=128, batch_size=32, num_workers=4, lr=1e-3,
        #                     start_epoch=0, num_epochs=101, device=device,
        #                     num_row=4, test_iter=10, gamma=1)

        # train_vae(dataset='cifar10', z_dim=128, batch_size=32, num_workers=4, lr=1e-3,
        #                     start_epoch=0, num_epochs=101, device=device,
        #                     num_row=4, test_iter=10, gamma=10)

        # AutoEncoder
        # train_vae(dataset='cifar10', path='/scratch/u7076589/project_Autoencoder/gamma001', z_dim=128, batch_size=32, num_workers=4, lr=1e-3,
        #                     start_epoch=0, num_epochs=101, device=device,
        #                     num_row=4, test_iter=10, gamma=0.01, vae=False)

        # train_vae(dataset='cifar10', path='/scratch/u7076589/project_Autoencoder/gamma01', z_dim=128, batch_size=32, num_workers=4, lr=1e-3,
        #                     start_epoch=0, num_epochs=101, device=device,
        #                     num_row=4, test_iter=10, gamma=0.1, vae=False)

        train_vae(dataset='cifar10', path='/scratch/u7076589/project_Autoencoder/gamma1', z_dim=128, batch_size=32,
                  num_workers=4, lr=1e-3,
                  start_epoch=0, num_epochs=101, device=device,
                  num_row=4, test_iter=10, gamma=1, vae=False)

        # train_vae(dataset='cifar10', path='/scratch/u7076589/project_Autoencoder/gamma10', z_dim=128, batch_size=32, num_workers=4, lr=1e-3,
        #                     start_epoch=0, num_epochs=101, device=device,
        #                     num_row=4, test_iter=10, gamma=10, vae=False)

        # VAE celebA
        # train_vae(dataset='celeb256', z_dim=256, batch_size=8, num_workers=4, lr=2e-4,
        #                     start_epoch=0, num_epochs=1001, device=device,
        #                     num_row=8, test_iter=10, gamma=0.5)
